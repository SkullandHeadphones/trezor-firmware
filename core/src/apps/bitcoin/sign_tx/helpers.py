...
if False:
    from typing import overload, Any, Awaitable, Dict, Union
...
@overload
def request_tx_output(tx_req: TxRequest, i: int, coin: CoinInfo) -> Awaitable[TxOutputType]:
    ...

@overload
def request_tx_output(tx_req: TxRequest, i: int, coin: CoinInfo, tx_hash: bytes) -> Awaitable[TxOutputBinType]:
    ...


def request_tx_output(tx_req: TxRequest, i: int, coin: CoinInfo, tx_hash: bytes = None) -> Awaitable[Union[TxOutputType, TxOutputBinType]]:  # type: ignore
    assert tx_req.details is not None
